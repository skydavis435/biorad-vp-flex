...
Documentation for the `calculate_vp()` function may be found here:
...
# Offer an option to save the plot
  ...
  downloadHandler(filename = function() {
    filename = paste0('manual_options_',basename(input$nex_file), ".png")
    },
    content = function(file) {
      png(file, width = 640, height = 480)
      plot(my_vp(), quantity = 'dbz',main = paste0('manual_options_',basename(input$nex_file))) 
      dev.off()
      }
    )
    ...
    datatable(extensions = list('Scroller' = NULL,'Buttons' = NULL),
              options = list(scrollX = TRUE,
                 ...
                 buttons = list(list(extend = 'csv',
                                     filename = paste0('manual_options_',basename(input$nex_file))),
                                list(extend= 'excel',
                                     filename = paste0('manual_options_',basename(input$nex_file)))
                                )
                 ),
